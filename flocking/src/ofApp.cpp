...
agent::agent(){	
  ...
  mouse.set(0,0);
  gravity.set(0,0.06); //initialize gravity
...
void agent::update(agent *arr){  // influence the main vector 
  ...
  if (middle != noVec&& !ofGetMousePressed()) {
  ...
  }else if(middle != noVec&& ofGetMousePressed()){
      float alignMouse = velDir.angle(location- middle);
      velDir.rotate(alignMouse*.1);
  }
...
void agent::draw(){
  ...
  ofSetColor(255,0,0);
    ...
    }// draw circle, which is from agent, if mouse not pressed
